  ...
  if(has("dom") && (has("dojo-inject-api") || has("dojo-dom-ready-api"))){
    ...
    if(has("dojo-inject-api")){
      ...
      req.injectUrl = function(url, callback, owner){
        ...
        var errorLoggingUrlPatterns = has("qbo-error-logging-url-patterns");
        if (url && errorLoggingUrlPatterns) {

          // Check the URL to see whether it matches the allowed URL patterns that were passed in
          // and set cross origin to anonymous for known domains only
          // Examples:
          // 1) new Regex("pattern").test("/c1/v89.069/scripts/dojo/bower_components/qbo-ui-libs-neo/src/NeoAppController.js")
          // 2) new Regex("pattern").test("https://cdn.qa.qbo.intuit.com/c1/v89.069/scripts/dojo/bower_components/qbo-ui-libs-neo/src/NeoAppController.js")
          // 3) new Regex("pattern").test("https://plugin-qbo-qa.intuitcdn.net/qbo-ui/10752/bower_components/qbo-ui-libs-neo/src/NeoAppController.js")
          // 4) new Regex("pattern").test("https://www.youtube.com/iframe_api")
          // crossorigin should be set to annonymous for 2) and 3) only because they are known URLs

          var siteOrigin = window.location.origin;
          if (!siteOrigin) {    // for IE
            siteOrigin = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ":" +  window.location.protocol : "");
          }

          if (url.indexOf(siteOrigin) !== 0) {
            var patternFound = new RegExp(errorLoggingUrlPatterns, "i").test(url);
            if (patternFound) {
